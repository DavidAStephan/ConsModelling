...
Spec 7b now in `run_all_specifications()`: identical structure to Spec 7
but uses `mortgage_payment_burden_rba` (the RBA E13 measured series)
instead of synthetic `mortgage_burden`. Sample auto-truncated to 2009Q1+
by `complete.cases`. Coefficients on the post-2009 sample diverge from
Spec 7 substantially (small n; nla_y goes negative; ecm_lag ≈ −0.236
vs Spec 7's −0.226). Catalogue spec for §8 measurement-quality
robustness column.

### NS-006 (original) Add Spec 7b: cohort + RBA-measured mortgage payment burden
...
Spec 10 implemented as `fit_williams_prior_spec()` in
`australia_estimation.R`. Imposes Williams' BIS Table 1 calibrations:
γ_IFA = 0.022, ψ_0 = 0.20, ψ_1 = 0.93, ϖ = 1.2. Solves via iterative
fixed-point OLS (16 iterations to converge on full sample). Uses our
canonical maximal-GETS `cci_williams` as the CCI input.

Result on full sample (1988Q4-2024Q4):
  λ          = -0.102 (vs Spec 8 unrestricted -0.121, vs Williams' -0.286)
  γ_HA       = +0.037 (significant t=2.65)
  γ_NLA      = +0.050 (insignificant t=1.14)
  γ_HP       = +0.002 (sign-violator)
  γ_r        = +0.0002 (sign-violator)
  pass_signs = TRUE

Diverges on the COVID-rich-dummies sample (handled via |λ| > 5
divergence safeguard returning NULL).

### NS-015 (original) Add a "Williams-prior" calibrated specification (Spec 9)

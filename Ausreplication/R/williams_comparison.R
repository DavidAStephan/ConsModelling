...
#     "CCI=peak" effects.
#
# Run: Rscript Ausreplication/R/williams_comparison.R
# ==============================================================================
...
williams <- tribble(
  ...
  "ha_y",                          0.0488,       0.0095,      5.14,    "Housing wealth MPC at CCI=0; falls to 0.0452 at peak",
...
ours <- tribble(
  ...
  "ha_y",                 o_ha$est,            abs_or_na(s_ha$est),  s_ha$t,        "Spec 6 (CCI=0 by construction)",
  ...
  "ifa_y",                if (!is.na(o_eq$est) && !is.na(o_sup$est)) o_eq$est + o_sup$est else NA_real_,
                          abs_or_na(ifa_struct), NA_real_,           "Spec 6 eq_y + super_y aggregated",
  "nla_y",                o_nla$est,           abs_or_na(s_nla$est), s_nla$t,       "Spec 6 NLA fix; restriction accepted",
...
md_lines <- c(
  ...
  sprintf("| Housing `ha_y` | %s | %s | %s | %s | %s%% | %s%% |",
          fmt(0.0488), fmt(0.0488 * 0.286), fmt(o_ha$est), fmt(abs_or_na(s_ha$est)),
  ...
  sprintf("| Net liquid `nla_y` | %s | %s | %s | %s | %s%% | %s%% |",
          fmt(0.159), fmt(0.159 * 0.286), fmt(o_nla$est), fmt(abs_or_na(s_nla$est)),
  ...
  "",
  "Generated by `Ausreplication/R/williams_comparison.R`. To refresh after",
  "re-estimation: `Rscript Ausreplication/R/williams_comparison.R`.",
  ""
)

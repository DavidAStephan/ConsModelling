...
| B3 | **De-meaned CCI interactions in Spec 8** | ✅ done | Implemented in `fit_consumption_with_williams_cci` / Spec 8 (`md8`): interacted variables are de-meaned over the estimation sample before forming each interaction. This made `ha_x_cci` right-signed (**+0.0016**, t = 0.32 — its theoretically correct positive value, though insignificant); see WP §8.4. |

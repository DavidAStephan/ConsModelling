read_abs_series_catalog <- function(catalog, path = Sys.getenv("R_READABS_PATH", unset = tempdir())) {
  split_catalog <- split(catalog, catalog$source_group)
  out <- list()

  for (group_name in names(split_catalog)) {
    group_catalog <- split_catalog[[group_name]]
    raw <- read_abs_series(series_id = group_catalog$series_id, path = path, check_local = TRUE)

    raw_names <- names(raw)
    lower_names <- tolower(raw_names)
    date_name <- raw_names[match("date", lower_names)]
    value_name <- raw_names[match("value", lower_names)]
    series_id_name <- raw_names[match("series_id", lower_names)]
    series_name_name <- raw_names[match("series", lower_names)]
    series_type_name <- raw_names[match("series_type", lower_names)]

    if (is.na(date_name) || is.na(value_name) || is.na(series_id_name)) {
      stop("read_abs_series() did not return the expected columns for source group ", group_name)
    }

    raw_tidy <- tibble(
      date = as.Date(raw[[date_name]]),
      value = as.numeric(raw[[value_name]]),
      series_id = as.character(raw[[series_id_name]]),
      source_series_name = if (!is.na(series_name_name)) as.character(raw[[series_name_name]]) else NA_character_,
      source_series_type = if (!is.na(series_type_name)) as.character(raw[[series_type_name]]) else NA_character_
    ) %>%
      filter(!is.na(date), !is.na(value), !is.na(series_id))

    for (i in seq_len(nrow(group_catalog))) {
      spec <- group_catalog[i, , drop = FALSE]
      out[[spec$object_name]] <- raw_tidy %>%
        filter(series_id == spec$series_id) %>%
        mutate(
          series_name = spec$series_name,
          series_type = spec$series_type
        ) %>%
        select(date, value, series_name, series_type, series_id) %>%
        arrange(date) %>%

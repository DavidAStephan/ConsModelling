...
- Downloads the exact ABS `series_id` values required for the model using `readabs::read_abs_series()`.
...
- The household balance-sheet series are current price stocks, so the script deflates them using a household consumption deflator constructed from ABS HFCE current-price and chain-volume series.
